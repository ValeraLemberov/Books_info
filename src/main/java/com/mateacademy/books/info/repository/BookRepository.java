...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    @Query("FROM Book b JOIN b.authors ba WHERE ba.authorName = :authorName")
    ...
    @Query("FROM Book b JOIN b.authors ba "
            ...
            + "FROM Book b JOIN b.authors ba WHERE ba.authorName = :authorName) "
            + "AND ba.authorName = :authorName")
    Book findBestSoldAmount(String authorName);

    @Query("FROM Book b JOIN b.authors ba "
            + "WHERE b.publishedAmount = (SELECT MAX(b.publishedAmount) "
            + "FROM Book b JOIN b.authors a WHERE a.authorName = :authorName) "
            + "AND ba.authorName = :authorName")
    ...
    @Query("SELECT DISTINCT b FROM Book b JOIN b.authors ba "
            ...
            + "FROM Book b JOIN b.authors ba WHERE ba.authorName "
            + "LIKE CONCAT('%', :authorName, '%')) "
            + "AND  ba.authorName LIKE CONCAT('%', :authorName, '%')")
    List<Book> findAllBySoldAmount(String authorName);

    @Query("SELECT DISTINCT b FROM Book b JOIN b.authors ba "
            + "WHERE b.publishedAmount IN (SELECT MAX(b.publishedAmount) "
            + "FROM Book b JOIN b.authors ba WHERE ba.authorName "
            + "LIKE CONCAT('%', :authorName, '%')) " + "AND ba.authorName "
            + "LIKE CONCAT('%', :authorName, '%')")
    ...
    @Query("FROM Book b JOIN b.authors ba WHERE (b.soldAmount/b.publishedAmount) IN "
            ...
            + "WHERE ba.authorName LIKE CONCAT('%',:authorName,'%')) "
            + "AND  ba.authorName LIKE CONCAT('%',:authorName,'%')")
